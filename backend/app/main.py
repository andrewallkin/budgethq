...
@app.post("/api/admin/trigger-daily-summary")
async def trigger_manual_daily_summary(
    request: dict,
    db: Session = Depends(database.get_db)
):
    """
    Manually trigger daily summary creation for a specific date.
    Checks if there are hourly snapshots for the date before proceeding.
    No authentication required for debugging purposes.
    """
    try:
        from datetime import date, datetime

        # Extract target_date from request body
        ...
        # Check if there are any portfolio value history records for this date
        start_of_day = datetime.combine(target, datetime.min.time())
        end_of_day = datetime.combine(target, datetime.max.time())

        existing_snapshots = db.query(models.PortfolioValueHistory).filter(
        ...
        if existing_snapshots == 0:
            return {
                "status": "no_data",
                "message": f"No hourly snapshots found for {target_date}. Cannot create daily summary.",
                "snapshots_found": 0
            }

        # Check if daily summary already exists
        existing_summary = db.query(models.DailyPortfolioSummary).filter(
            models.DailyPortfolioSummary.date == target
        ).first()

        if existing_summary:
            return {
                "status": "already_exists",
                "message": f"Daily summary already exists for {target_date}",
                "snapshots_found": existing_snapshots
            }

        # Create the daily summary
        stats = history.create_daily_summary(db, target)

        return {
            "status": "success",
            "message": f"Daily summary created for {target_date}",
            "stats": stats,
            "snapshots_found": existing_snapshots
        }
        ...
        raise HTTPException(status_code=500, detail=f"Error during daily summary creation: {str(e)}")
