...
export default function InvestmentsLanding() {
    ...
    const fxRateSourceLabel = (source) => {
        if (source === 'google_sheets') return 'Google Sheet'
        if (source === 'google_sheets_cached_stale') return 'Google Sheet (cached — refresh failed)'
        return source ? String(source) : 'sheet'
    }
        ...
        <div className="space-y-6">
                ...
                <div className="mt-5">
                    ...
                    {totalValueBaseCurrency != null ? (
                        <>
                            ...
                            {(fxSummary?.as_of || fxSummary?.sheet_error) && (
                                <div className="mt-2 space-y-1 text-sm text-emerald-100/95 leading-snug">
                                    ...
                                    {fxSummary?.sheet_error ? (
                                        <p className="text-emerald-50/95 text-xs">{fxSummary.sheet_error}</p>
                                    ) : null}
                                </div>
                            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                {portfolios.map((portfolio) => (
                    <div key={portfolio.id} className="bg-white dark:bg-gray-800 p-5 rounded-xl border border-gray-200 dark:border-gray-600 shadow-sm">
                        ...
                        <Link
                            ...
                            className="mt-4 inline-flex items-center justify-center w-full px-4 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700"
                        >
                            Open Portfolio
                        </Link>
                    </div>
